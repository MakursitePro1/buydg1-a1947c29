...
const RotateTool = () => {
  ...
  const rotate = (deg: number) => { const r = (rotation + deg + 360) % 360; setRotation(r); if (image) draw(image, r, flipH, flipV); };
  const toggleFlipH = () => { setFlipH(!flipH); if (image) draw(image, rotation, !flipH, flipV); };
  const toggleFlipV = () => { setFlipV(!flipV); if (image) draw(image, rotation, flipH, !flipV); };
