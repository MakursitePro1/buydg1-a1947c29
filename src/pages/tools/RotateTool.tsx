...
const RotateTool = () => {
  ...
  const onImageLoad = useCallback((img: HTMLImageElement) => {
    ...
    setFlipH(false);
    setFlipV(false);
    draw(img, 0, false, false);
  }, []);

  const draw = (img: HTMLImageElement, rot: number, fh: boolean, fv: boolean) => {
  ...
  const rotate = (deg: number) => {
    ...
    setRotation(newRot);
    if (image) draw(image, newRot, flipH, flipV);
  };

  const toggleFlipH = () => {
    setFlipH(!flipH);
    if (image) draw(image, rotation, !flipH, flipV);
  };
        ...
        <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
          <div className="space-y-4 rounded-xl border bg-card p-5">
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button variant="outline" onClick={toggleFlipH} className={flipH ? "border-accent text-accent" : ""}>
                <FlipHorizontal className="mr-2 h-4 w-4" /> Flip H
              </Button>
              <Button variant="outline" onClick={toggleFlipV} className={flipV ? "border-accent text-accent" : ""}>
